...
class Bad_Checks implements themecheck {
	...
	function check( $php_files, $css_files, $other_files ) {
		...
		$checks = array(
			...
			'/\s?ini_set\(/' => __( 'Plugins should not change server PHP settings', 'theme-check' ),
		...
		foreach ( $other_files as $php_key => $phpfile ) {
			foreach ( $checks as $key => $check ) {
				...
				if ( preg_match( $key, $phpfile, $matches ) ) {
					...
					$grep = tc_grep( $error, $php_key );
					$this->error[] = sprintf(__('<span class="tc-lead tc-warning">REQUIRED</span>: Found <strong>%1$s</strong> in the file <strong>%2$s</strong>. %3$s.%4$s', 'theme-check'), $error, $filename, $check, $grep);
					$ret = false;
				}
			}
		}
